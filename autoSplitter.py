...
def getNewFunc(_fileName):
	proc = subprocess.Popen("./mips_to_c/mips_to_c.py "+optionalArgs+" "+_fileName+" "+getFileName(_fileName),
		shell=True,
		stdout=subprocess.PIPE)
...
for i in sFiles:
	...
	for sym in symReference:
		...
		oFile.write(getNewFunc(sys.argv[1]+"/asm/non_matchings/"+getFileName(i)+"/"+sym+".s "))
		oFile.write("#else\n")
		oFile.write("GLOBAL_ASM(\"asm/non_matchings/"+getFileName(i)+"/"+sym+".s\")\n")
		oFile.write("#endif\n\n")
	if len(symReference) == 0:
		oFile.write("GLOBAL_ASM(\"asm/non_matchings/"+getFileName(i)+"/"+getFileName(i)+".s\")\n")
	symReference={}
	curr_sym=""
	sectionStart = 0
	fileStartAddr = ""
